...
SELECT od.* FROM order_details od WHERE od.pizza_id IN (SELECT p.id FROM pizzas p WHERE p.price > 20);
...
SELECT od.pizza_id, SUM(od.quantity * p.price) AS total_revenue FROM order_details od JOIN pizzas p ON od.pizza_id = p.id GROUP BY od.pizza_id;
-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date, COUNT(od.id) AS order_count, SUM(od.quantity) AS total_quantity FROM orders o JOIN order_details od ON o.id = od.order_id GROUP BY o.date ORDER BY o.date;
-- # 고급 문제
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT p.*, SUM(od.quantity) AS total_quantity FROM pizzas p JOIN order_details od ON p.id = od.pizza_id GROUP BY p.id ORDER BY total_quantity DESC LIMIT 10;
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT o.date, COUNT(o.id) AS total_orders, SUM(od.quantity * p.price) AS total_amount FROM orders o JOIN order_details od ON o.id = od.order_id JOIN pizzas p ON od.pizza_id = p.id WHERE o.date >= '2025-03-01' AND o.date < '2025-04-01' GROUP BY o.date ORDER BY o.date;
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
SELECT pt.name AS pizza_name, p.size AS pizza_size, p.price AS pizza_price, od.quantity, (od.quantity * p.price) AS total_amount FROM orders o JOIN order_details od ON o.id = od.order_id JOIN pizzas p ON od.pizza_id = p.id JOIN pizza_types pt ON p.type_id = pt.id WHERE o.id = 78;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size AS pizza_size, SUM(od.quantity * p.price) AS total_revenue FROM order_details od JOIN pizzas p ON od.pizza_id = p.id GROUP BY p.size
ORDER BY CASE 
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT pt.name AS pizza_type, SUM(od.quantity * p.price) AS total_revenue FROM order_details od JOIN pizzas p ON od.pizza_id = p.id JOIN pizza_types pt ON p.type_id = pt.id GROUP BY pt.name ORDER BY total_revenue DESC;