...
SELECT o.date, COUNT(DISTINCT o.order_id) AS total_orders, SUM(p.price * od.quantity) AS total_amount
FROM orders o
JOIN order_details od ON o.order_id = od.order_id
...
SELECT pt.name AS pizza_name, p.size AS pizza_size, p.price AS pizza_price, od.quantity, p.price * od.quantity AS total_amount
