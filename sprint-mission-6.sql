...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT o.pizza_id , p.type_id, p.size, p.price, SUM(o.quantity) AS total_quantity
FROM order_details o
JOIN pizzas p ON p.id = o.pizza_id
GROUP BY o.pizza_id, p.type_id, p.size, p.price
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT o.date, SUM(od.quantity) AS total_orders, SUM(od.quantity*p.price) AS total_amount
FROM orders o
